...
struct RawAdapterNames
{
    char *easy_name;
    char *hard_name;
};

struct RawAdapterNames adapter_names[64];
 ...
 ***************************************************************************/
void
rawsock_ignore_transmits(struct RawAdapter *adapter, const unsigned char *adapter_mac)
{
    if (adapter->ring) {
 ...
 ***************************************************************************/
static void
rawsock_close_adapter(struct RawAdapter *adapter)
{
    if (adapter->ring) {
                     ...
                     const char *bpf_filter)
{
    ...
    adapter = (struct RawAdapter *)malloc(sizeof(*adapter));
    if (adapter == NULL)
        exit(1);
    memset(adapter, 0, sizeof(*adapter));
    adapter->is_packet_trace = is_packet_trace;
    adapter->pt_start = 1.0 * time(0);
 ...
 ***************************************************************************/
int
rawsock_selftest_if(const char *ifname, unsigned is_reuse)
{
    int err;
    unsigned ipv4 = 0;
